...
filename='Flexión a 3 puntos 6 Vidrios 4 Maderas Materiales 2 GrA Abril 22 de 2026.csv';
opts = detectImportOptions(filename);
opts.DataLines = 4; 
opts.VariableNames = {'Tiempo', 'Fuerza', 'Desplazamiento'};
datos= readtable(filename, opts);

fuerza_N = datos.Fuerza;
deltaL_mm =  datos.Desplazamiento;

fuerza_N = datos.Fuerza - datos.Fuerza(1);
deltaL_mm = datos.Desplazamiento - datos.Desplazamiento(1);
%---solicitud datos iniciales---

preguntas = {'Espesor inicial de la probeta (en mm): ', 'Ancho inicial de la probeta (en mm): ', ...
             'Distancia entre apoyos medida (en mm): '};
titulo = 'Configuración inicial del ensayo';
lineas= 1;

solicitud=inputdlg(preguntas,titulo,lineas);

h = str2double(solicitud {1});
b= str2double(solicitud {2});%---leer datos (cargas y deltaL)---

L_i = str2double(solicitud{3});

if isempty(solicitud)
    error('Operación cancelada');
end

if isnan(h) || isnan(b) || isnan(L_i)
...
Esfuerzo = (3 * fuerza_N * L_i) ./ (2 * b * h^2);
Deformacion = (6 * deltaL_mm * h) ./ (L_i^2);
